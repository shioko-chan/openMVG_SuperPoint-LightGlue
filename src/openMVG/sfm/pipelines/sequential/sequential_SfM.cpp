...
namespace openMVG
{
  namespace sfm
  {
    ...
    bool SequentialSfMReconstructionEngine::InitLandmarkTracks()
    {
      ...
      OPENMVG_LOG_INFO << map_tracks_.size() << " tracks have been found";
...
#pragma omp critical
      ...
      for (size_t i = 1; i < vec_putative.size() &&
                         vec_putative[i].second > threshold;
           ++i)
      {
    ...
    bool SequentialSfMReconstructionEngine::Resection(const uint32_t viewIndex)
    {
      ...
      // We use a local scene with only the 3D points and the new camera.
      {
        ...
        if (b_new_intrinsic)
        {
          ...
          case PINHOLE_CAMERA:
            optional_intrinsic =
                std::make_shared<Pinhole_Intrinsic>(view_I->ui_width, view_I->ui_height, focal, principal_point(0), principal_point(1));
            break;
          case PINHOLE_CAMERA_RADIAL1:
            optional_intrinsic =
                std::make_shared<Pinhole_Intrinsic_Radial_K1>(view_I->ui_width, view_I->ui_height, focal, principal_point(0), principal_point(1));
            break;
          case PINHOLE_CAMERA_RADIAL3:
            optional_intrinsic =
                std::make_shared<Pinhole_Intrinsic_Radial_K3>(view_I->ui_width, view_I->ui_height, focal, principal_point(0), principal_point(1));
            break;
          case PINHOLE_CAMERA_BROWN:
            optional_intrinsic =
                std::make_shared<Pinhole_Intrinsic_Brown_T2>(view_I->ui_width, view_I->ui_height, focal, principal_point(0), principal_point(1));
            break;
          case PINHOLE_CAMERA_FISHEYE:
            optional_intrinsic =
                std::make_shared<Pinhole_Intrinsic_Fisheye>(view_I->ui_width, view_I->ui_height, focal, principal_point(0), principal_point(1));
            break;
          default:
